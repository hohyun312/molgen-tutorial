...
class Environment:
    ...
    def Action_to_idx(self, state: State, action: Action) -> int:
        y = None
        ...
        if action is not None:
            if action.type == ActionType.STOP:
                ...
                y = (
                    self.num_node_types
                    + self.num_edge_types * non_edges.index(edge)
    ...
    def idx_to_Action(self, state: State, idx: int) -> Action:
        action = None
        if idx == 0:
            action = Action(ActionType.STOP)

        elif idx - 1 < self.num_node_types:
            action = Action(type=ActionType.AddNode, node_type=NodeType(idx))
        else:
            idx = idx - 1 - self.num_node_types
            i = idx // self.num_edge_types
            t = idx % self.num_edge_types
            non_edges = state.get_non_edge_list()
            src, tgt = non_edges[i]
            action = Action(
    ...
    def get_bfs_trajectory(self, state):
        ...
        def take_action(action):
            next_cur, *_ = self.step(cur, action)
